...
int main(int argc, char *argv[])
{
    ...
    if (strcmp(type, "local") == 0)
    {
        ...
        server_socket_binded = socket(AF_UNIX, SOCK_DGRAM, 0);
        ...
        bind(server_socket_binded, (struct sockaddr *)&binded_sockaddr,
             sizeof(struct sockaddr_un));
    ...
    if (strcmp(type, "local") == 0)
    {
        sendto(server_socket_binded, buffer, MAX_MESSAGE_LENGTH, 0, (struct sockaddr*)&local_sockaddr, sizeof(struct sockaddr_un));
    }
    ...
    while (1)
    {
        if (strcmp(type, "local") == 0)
        {
            recv(server_socket_binded, buffer, MAX_MESSAGE_LENGTH, 0);
        }
        token = strtok(buffer, " ");
        command = strtok(NULL, " ");
